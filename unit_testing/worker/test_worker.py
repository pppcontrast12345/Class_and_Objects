...
#from unit_testing.worker.worker import Worker
...
class WorkerTests(TestCase):
    ...
    def test_correct_init(self):
        self.assertEqual("Andrew", self.worker.name)
        self.assertEqual(1000, self.worker.salary)
        self.assertEqual(100, self.worker.energy)
        self.assertEqual(0, self.worker.money)

    def test_work_when_worker_has_zero_energy_raises_exception_error(self):
    ...
    def test_rest_increase_energy_by_one(self):
    ...
    def test_get_info_returns_correct_string(self):
        self.assertEqual('Andrew has saved 0 money.', self.worker.get_info())
